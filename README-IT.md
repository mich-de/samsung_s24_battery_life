...
## ⚡ Comandi ADB manuali (cosa l'app non può fare)

L'app fa tutto tramite Shizuku. Questi passaggi manuali servono solo una volta per l'installazione:

### Avviare Shizuku (necessario per l'app)

```powershell
# Via ADB (telefono collegato a PC con USB Debugging):
.\adb.exe shell sh /data/local/tmp/shizuku.dex
```

### Abilitare Wireless Debugging (alternativa a USB)

```powershell
# Pair con l'app Shizuku:
.\adb.exe shell settings put global development_settings_enabled 1
```

Tutto il resto — disabilitare pacchetti, cambiare impostazioni, limitare app in background — si fa dentro l'app.
...
Se dopo l'ottimizzazione noti ancora drain anomalo da Play Services (verifica in Impostazioni > Batteria), esegui da terminale ADB:

```powershell
.\adb.exe shell am force-stop com.google.android.gms
.\adb.exe shell pm clear com.google.android.gms
```
...
```
s24-battery-optimizer/
├── releases/                   ← Cartella contenente l'app Android
...
├── README.md                   ← Documentazione in inglese (Default)
...
**Perdo i dati?** No. `pm disable-user` solo nasconde le app. Con l'app puoi riattivarle.

**Google Pay / Wallet funziona?** Sì, a meno che non esegui `pm clear` su GMS.

**Samsung Pay?** L'app non tocca `spayfw`. Se lo disabiliti manualmente, Samsung Wallet non funzionerà.

**Bixby non funziona più?** Corretto. Se ti serve Bixby, non disabilitare i pacchetti Bixby nell'app.
