...
## ⚡ Comandos ADB manuais (o que o app não pode fazer)

O app faz tudo via Shizuku. Estes passos manuais são necessários apenas uma vez para a instalação:

### Iniciar Shizuku (necessário para o app)

```powershell
# Via ADB (telefone conectado ao PC com USB Debugging):
.\adb.exe shell sh /data/local/tmp/shizuku.dex
```

### Habilitar Wireless Debugging (alternativa ao USB)

```powershell
# Pair com o app Shizuku:
.\adb.exe shell settings put global development_settings_enabled 1
```

Todo o resto — desabilitar pacotes, alterar configurações, restringir apps em segundo plano — é feito dentro do app.
---
...
```
s24-battery-optimizer/
├── releases/                   ← Diretório contendo o app Android
...
**Vou perder meus dados?** Não. `pm disable-user` apenas oculta os aplicativos. Você pode restaurá-los pelo app.

**O Google Pay / Wallet funciona?** Sim, a menos que você limpe os dados do GMS.

**O Samsung Pay funciona?** O app não toca no `spayfw`. Se você desativá-lo manualmente, o Samsung Wallet deixará de funcionar.

**O Bixby não funciona mais?** Correto. Se você precisa do Bixby, não desative os pacotes do Bixby no app.
