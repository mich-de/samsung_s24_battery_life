...
class ShizukuBridgeProvider : ContentProvider() {
    ...
    /**
     * The provider has to stay exported so Shizuku can hand us the binder, but `getBinder`
     * would otherwise pass shell-level privileges to any app that queries this authority.
     *
     * The check is by UID, not by package: the Shizuku server runs as `shell` (or root),
     * and asking `getCallingPackage()` for a caller whose package does not belong to its
     * UID throws SecurityException, which would reject the one caller we must accept.
     */
    private fun callerAllowed(): Boolean {
        val uid = Binder.getCallingUid()
        if (uid == Process.myUid() || uid == SHELL_UID || uid == ROOT_UID) return true
        val shizukuUid = try {
            context?.packageManager?.getPackageUid(SHIZUKU_PKG, 0) ?: -1
        } catch (_: Exception) {
            -1
        }
        return uid == shizukuUid
    }
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        ...
        if (!callerAllowed()) {
            Log.w("ShizukuBridge", "Rejected call('$method') from uid ${Binder.getCallingUid()}")
            return null
        }
